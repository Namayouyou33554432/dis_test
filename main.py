        ...
        async with message.channel.typing():
            ...
            async with aiohttp.ClientSession(headers=headers) as session:
                if url_type == 'twitter':
                    ...
                    # ★★★ 方法1: pxiv.catからURLを推定 (複数枚に強い) ★★★
                    print(f"Pixiv: Attempting URL guessing for {artwork_id}")
                    for i in range(1, 21):
                        ...
                        for ext in ['.jpg', '.png', '.gif']:
                            ...
                            try:
                                ...
                                # ログには残すが、処理は続行
                                print(f"URL guessing for {img_url} failed: {e}")
                        if not found_image_for_this_page:
                            break
                    
                    # ★★★ 方法2: 方法1が失敗した場合、埋め込みから取得 (予備) ★★★
                    if not image_urls:
                        print("Pixiv: URL guessing failed. Falling back to embed check.")
                        try:
                            await asyncio.sleep(3) # 埋め込み生成を待つ
                            updated_message = await message.channel.fetch_message(sent_mirror_message.id)
                            if updated_message.embeds:
                                for embed in updated_message.embeds:
                                    if embed.image and embed.image.url:
                                        image_urls.append(embed.image.url)
                        except Exception as e:
                            print(f"Embed fallback check failed: {e}")
